...
float distance(int x,int y)// calculate distance
{
    float a=8-x;
    float b=8-y;
...
int main()
{
    ...
    int x=0,y=0; // starting index of the robo
    char p,key;
    ...
    while(x!=8 || y!=8)
    {
        ...
        if (ld==xx && rd==xx && fd>xx ) 
        ...
        else if (ld>xx && rd>xx && fd==xx )
            {
                ...
                float Left_distance=distance(x1,y1);// checking distance for left option
                float Right_distance=distance(x2,y2); // checking distance for right option
        ...
        else if (ld>xx && rd==xx && fd>xx )
            {
              ...
              Rotation(fnx1,2,x2,y2);
                float Left_distance=distance(x1,y1);
                float Forward_distance=distance(x2,y2);
        ...
        else if (ld==xx && rd>xx && fd>xx )
            {
               ...
               fnx1=fnx;
               Rotation(fnx1,2,x2,y2);
                float Right_distance=distance(x1,y1);
                float Forward_distance=distance(x2,y2);
        ...
        else if (ld>xx && rd>xx && fd>xx )
            {
              ...
              float Left_distance=distance(x1,y1);
              float Right_distance=distance(x2,y2);
              float Forward_distance=distance(x3,y3);
    ...
    int k=string_index;
    string st1[k]; //revers string for come back to starting point 
     for(int i=0;i<string_index;i++)
        {
            if(st[k-2]=='L')
            {
